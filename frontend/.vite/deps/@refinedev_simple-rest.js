...
var q = (r, s = y) => ({ getList: async ({ resource: e, pagination: t, filters: o, sorters: n, meta: d }) => {
  let i = `${r}/${e}`, { current: c = 1, pageSize: a = 10, mode: p = "server" } = t ?? {}, { headers: $, method: m } = d ?? {}, x = m ?? "get", M = f(o), u = {};
  p === "server" && (u._start = (c - 1) * a, u._end = c * a);
  let _ = h(n);
  if (_) {
    let { _sort: T, _order: b } = _;
    u._sort = T.join(","), u._order = b.join(",");
  }
  let v = { ...u, ...M }, O = Object.keys(v).length ? `${i}?${(0, import_query_string2.stringify)(v)}` : i, { data: w, headers: F } = await s[x](O, { headers: $ }), S = +F["x-total-count"];
